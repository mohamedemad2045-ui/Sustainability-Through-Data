...
ORDER BY total_impact_million_USD DESC;

-- ----------------------------------------------
-- Response time vs. deaths by severity category
-- ----------------------------------------------
...
SELECT
    ...
    COUNT(*)                                        AS event_count,
    ROUND(AVG(rt.response_time_hours), 2)           AS avg_response_time_hr,
    MIN(rt.response_time_hours)                     AS min_response_hr,
    MAX(rt.response_time_hours)                     AS max_response_hr,
    ROUND(AVG(f.deaths), 2)                         AS avg_deaths,
    ROUND(AVG(f.economic_impact_million_usd), 2)    AS avg_economic_impact_million_USD
FROM fact_global_climate_events f
JOIN dim_severity     sv ON f.severity_category_id = sv.severity_category_id
...
-- -------------------------------------------
-- Most Economically Burdened Populations
-- -------------------------------------------
SELECT 
    l.country,
    l.continent,
...
-- ------------------------------------------------
-- Total Casualties vs Aid Percentage by Event Type
-- -----------------------------------------------
SELECT 
    et.event_type,
    et.event_family,
...
-- -------------------------------------------
-- High Impact Per Capita but Low Aid Coverage
